...
import { validateOfflineSuscriptionOrder } from '@/utils/Endpoints/adminEndpoints';
...
export const FormValidateOfflineSuscription = ({ status, userId, orderId }: Props) => {
  ...
  const onSubmit = async (data: Record<string, any>) => {
    ...
    try {
      const res = await validateOfflineSuscriptionOrder(newData);
    ...
    <div>
            ...
            <div className="flex flex-col md:flex-row justify-start items-start md:items-center">
              ...
              <TableCell className="py-4 text-right dark:text-white">
                {!isLoading ? (
                  ...
                  >
                    {status === 'Pagado' ? 'Abonado' : 'Marcar como pagado'}
                  </Button>
                ) : (
      ...
      {status === 'Pagado' && (
        <div className="text-sm text-gray-500 ">
          <p>La orden de compra se ha completado y le has dado acceso al nuevo curso</p>
          <p>Si crees que se trató de un error, contactate con el equipo de soporte</p>
        </div>
      )}
      {isApproved && (
        <Alert
