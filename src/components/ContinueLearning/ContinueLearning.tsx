      ...
      {continueLearningCourses && continueLearningCourses.data?.length > 0 && (
        <div>
          <div className="pb-3 flex flex-col md:flex-row justify-center md:justify-between">
            <MediumTitle title="Continuar aprendiendo" />
          ...
          <section className="sm:mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-9 sm:gap-9">
              {continueLearningCourses.data.map((course, index) => (
                <Card
                  key={index}
                  className="w-full max-w-md ml-0 flex-shrink-0 flex flex-col dark:bg-card bg-white overflow-hidden shadow-md rounded-xl sm:rounded-2xl transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-lg py-4 sm:py-0 snap-start"
                >
                  <CardContent className="flex flex-col h-full p-0">
                    ...
                    <div className="sm:p-4 px-2 pb-4 pt-0 md:pt-4 flex items-center gap-2 sm:gap-3">
                      ...
                      <div className="flex-1 min-w-0 dark:text-white text-black">
                        <h3 className="font-medium text-xs md:text-sm xl:text-base truncate">
                          {course.title}
                        </h3>
                    ...
                    <div className="p-2 sm:p-4 flex flex-col flex-grow ml-1 sm:ml-2">
                      <div className="sm:mb-2 dark:text-white text-black">
                        ...
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm sm:text-base lg:text-lg font-medium truncate pt-2">
                      ...
                      <div className="flex items-start gap-1 sm:gap-2 mt-1 sm:mt-2 dark:text-white text-black pb-1 sm:pb-2">
                        <Link
                          href={
                            course?.nextLesson?.videoUrl
                              ? `/curso/${course.courseId}?videoUrl=${course?.nextLesson?.videoUrl
