...
export async function POST(request: Request) {
  try {
    ...
    if (existingProgress) {
      // Si el progreso ya existe, asegurarse de agregar el curso al campo `enrolledCourses`
      ...
      return NextResponse.json(
        {
          message: `El progreso del curso ya existe para este usuario. Se habilitó al usuario para acceder al curso ${courseId}`,
          success: true,
        },
        { status: 200 }
      );
    ...
    // Agregar el curso al campo `enrolledCourses` del usuario
    user.enrolledCourses.push(courseId);
    await user.save();
    ...
    return NextResponse.json(
      {
        message:
          "Progreso del curso creado correctamente y usuario habilitado para acceder a él",
        success: true,
