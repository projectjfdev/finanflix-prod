...
export async function POST(req: Request) {
  ...
  try {
    let mailOptions: any = {
      from: AUTH_EMAIL,
      to: AUTH_EMAIL,
      subject: `El usuario ${username} ha cancelado una suscripción - "${name}"`,
      html: `
<!DOCTYPE html>
<html lang="es">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
          <tr>
            <td style="padding: 20px; text-align: center; background-color: #4C2FBA;" class="email-header">
              <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: bold;">El usuario ${username} ha cancelado una suscripción - "${name}"
              </h1>
            </td>
          </tr>
          <tr>
