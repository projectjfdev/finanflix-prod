  ...
  return (
    <div className="min-h-screen dark:bg-card bg-background pb-5 pt-5 md:pt-10">
      <div className="container md:px-4 mx-auto">
        <div className="text-center mb-7 px-4 md:px-0">
          <BigTitle
            className="text-[24px] sm:text-2xl lg:text-3xl xl:text-4xl font-semibold dark:text-white text-black"
        ...
        >
          <TabsList className="grid w-full grid-cols-3 mb-8 gap-3 text-center justify-center px-4 md:px-0">
            {plans?.map(plan => (
              <TabsTrigger
                key={plan.name.toLowerCase()}
                value={plan.name.toLowerCase()}
                className={`${
                  cicloPago === plan.name.toLowerCase()
                ...
                } text-sm py-3 sm:text-base dark:hover:text-gray-300 hover:text-gray-800`}
              >
                {plan.name}
              </TabsTrigger>
            ))}
          ...
          <div className="grid grid-cols-1 my-10 justify-center mx-auto w-full px-4">
            {plans?.map(plan => (
              <TabsContent key={plan.name.toLowerCase()} value={plan.name.toLowerCase()}>
                <Card className="flex flex-col dark:bg-background bg-white dark:text-white text-black overflow-y-hidden transition-all duration-300  h-full max-w-xl mx-auto">
                  <CardHeader className="py-6 px-14 flex-shrink-0">
                    <div className="space-y-2">
                      <CardTitle className="text-3xl font-bold dark:text-white text-black">
                        {plan.name}
                      </CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="py-6 px-14 pt-0 flex-grow flex flex-col">
                    <div className="text-2xl lg:text-4xl  font-bold mb-8">
                      <span className="text-3xl lg:text-4xl xl:text-[44px]  font-bold">
                        ...
                        {plan.name.toLowerCase() === 'mensual' ? (
                          <span>
                            ...
                            <span className="text-sm">
                              {' '}
                              luego
                              {typeof plan?.price?.basic === 'number' && typeof rate === 'number'
                                ? ' ' +
                                  Intl.NumberFormat('es-AR').format(plan.price.basic * rate) +
                                  getPeriodoTexto(plan.name.toLowerCase() as CicloPago)
                                : ' Calculando...'}
                            </span>
                          </span>
                        ) : typeof plan?.price?.basic === 'number' && typeof rate === 'number' ? (
                          'AR$' + new Intl.NumberFormat('es-AR').format(plan.price.basic * rate)
                        ) : (
                          'Calculando...'
                        )}
                      </span> */}
                  ...
                  <CardFooter className="p-6 mt-auto flex-shrink-0">
                    <Button
                      ...
                      className={`w-full py-6 text-base font-semibold dark:hover:bg-secondary`}
                    >
                      Comenzar
                    </Button>
                  </CardFooter>
