// C:\Users\Ecotech\Desktop\Jeronimo Alderete\finanflix-prod-main\src\lib\discord-roles-suscription.ts
...
// Función para obtener el ID de rol según la suscripcion
export function getRoleIdForExpiredSuscriptions(subscriptionType: string): string | null {
  ...
  const subscriptionRoleMap: Record<string, string> = {
    ...
    'Suscripcion icon - mensual': process.env.NEXT_PUBLIC_DISCORD_ROLE_ICON_VENCIDO || '',
    'Suscripcion icon - semestral': process.env.NEXT_PUBLIC_DISCORD_ROLE_ICON_VENCIDO || '',
    'Suscripcion icon - anual': process.env.NEXT_PUBLIC_DISCORD_ROLE_ICON_VENCIDO || '',

    'Suscripcion diamond - mensual': process.env.NEXT_PUBLIC_DISCORD_ROLE_DIAMOND_VENCIDO || '',
    'Suscripcion diamond - semestral': process.env.NEXT_PUBLIC_DISCORD_ROLE_DIAMOND_VENCIDO || '',
    'Suscripcion diamond - anual': process.env.NEXT_PUBLIC_DISCORD_ROLE_DIAMOND_VENCIDO || '',
  };
...
export async function startSuscriptionSequence(
  userId: string,
  ...
  try {
    ...
    const suscriptionRoleId = getRoleIdForSubscription(subscriptionType);
    ...
    const expiredRoleId = getRoleIdForExpiredSuscriptions(subscriptionType);
    ...
    setTimeout(async () => {
      try {
        ...
        const expiredAssignResult = await assignSuscriptionRoleToUser(userId, expiredRoleId);
...
export async function processUserSuscription(
  userId: string,
  ...
  for (const subscriptionType of subscriptionTypes) {
    ...
    if (!expiredRoleId) {
      ...
      // );
      await startSuscriptionSequence(userId, subscriptionType);
      continue;
    }
...
export async function removeSubscriptionExpiredRole(
  userId: string,
  subscriptionType: string
): Promise<void> {
  try {
